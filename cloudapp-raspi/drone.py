...
### use this for mavlink pixhawk -> https://youtu.be/BSDST7ePOtM
...
class Drone:
    def __init__(self, configurations):
        ...
        if use_simulator:
            ...
            ### if on wifi network
            rpi_ip = netifaces.ifaddresses('wlan0')[netifaces.AF_INET][0]['addr']
    ...
    def executeCommand(self, command):
        ...
        if command.code == 1:
            ...
            logging.debug('Executing Code: %s for Command: %s', str(command.code), 'Increase Vertical Speed')
            return
        if command.code == 5:
            self.control_tab.decreaseSpeedZ()
            logging.debug('Executing Code: %s for Command: %s', str(command.code), 'Decrease Vertical Speed')
            return
        if command.code == 2:
